...
def is_set_by_name(text: str) -> bool:
    return not bool(
        re.match(r"(#|0x)?([0-9a-f]{3}|[0-9a-f]{6})", text.strip(), flags=re.IGNORECASE)
    )
...
class CBoxRole(SphinxRole):
    def run(self) -> Tuple[List[Node], List[system_message]]:
        ...
        box = CBoxNode(
            value=col,
            set_by_name=is_set_by_name(self.text),
        )
        return [box], []
...
class CBoxNode(nodes.General, nodes.Element):
    def hexval(self, prefix) -> str:
        return self["value"].format_value(prefix)

    def cname(self) -> str:
        return self["value"].name or "<unnamed>"

    def is_set_by_name(self) -> bool:
        return self["set_by_name"]

    @staticmethod
    def _compose_html_colorbox_span(hexval: str, titlestr: str | None) -> str:
        span = '<span class="colorbox" %s>&nbsp;</span>'
        attrs = [
            ' style="background-color: %s"' % hexval,
        ]
        if titlestr is not None:
            attrs.append(f'title="%s"' % titlestr)
        return span % " ".join(attrs)

    @staticmethod
    def visit_node_html(self, node: ColorBoxNode):
        hexval = node.hexval("#")
        title = [hexval]
        if node.is_set_by_name():
            title.append(node.cname())
        titlestr = " ".join(title)

        span = node._compose_html_colorbox_span(hexval, titlestr)
    ...
    def visit_node_latex(self, node):
        ...
        self.body.append(
            "\\begin{ptcolorbox}[colback=HEX%s,colupper=HEX%s]{}\\end{ptcolorbox}"
            % (hexval, hexval)
        )
...
class ColorBoxNode(CBoxNode):
    ...
    def visit_node_html(self, node):
        ...
        if node.is_set_by_name():
            spans.append(f'<span class="colorboxlabel literal">{node.cname()}</span>&nbsp;')
            title.append(hexval)

        titlestr = " ".join(title) if title else None
        spans.append(node._compose_html_colorbox_span(hexval, titlestr))

        if not node.is_set_by_name():
            spans.append(f'<code class="colorboxvalue hex literal">{node.hexval("#")}</code>')
    ...
    def visit_node_latex(self, node):
        if node.is_set_by_name():
            self.body.append("\\emph{%s}\\space" % node.cname())
...
def _patch_add_icon(
    node_cls: type[IconNode], elem_classes: tuple[type[SphinxRole | Directive], ...]
):
