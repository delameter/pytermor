...
def format_si_metric(value: float) -> str:
    ...
    .. versionadded:: 2.0
    """
    return format_prefixed_unit(value, PRESET_SI_METRIC)
...
def format_si_binary(value: float) -> str:
    ...
    .. versionadded:: 2.0
    """
    return format_prefixed_unit(value, PRESET_SI_BINARY)


def format_prefixed_unit(value: float, preset: PrefixedUnitPreset) -> str:
    """
    ...
    while 0 <= unit_idx < len(prefixes):
        ...
        if preset.integer_input and unit_idx == preset.prefix_zero_idx:
            ...
            num_str = format_auto_float(value,
                                        preset.max_value_len,
                                        allow_exponent_notation=False)

        return f'{num_str.strip()}{unit_separator}{unit_full}'.strip()

    # no more prefixes left
    num_str = format_auto_float(value,
                                preset.max_value_len,
                                allow_exponent_notation=False)
    unit_full = ('?' * max([len(p) for p in prefixes if p])) + (preset.unit or '')
    return f'{num_str.strip()}{unit_separator}{unit_full}'.strip()
