...
.. testsetup:: *
    ...
    from pytermor import build, SequenceSGR, Spans, Span, Seqs, IntCodes
...
def build(*args: str | int | SequenceSGR) -> SequenceSGR:
    ...
    Each sequence param can be specified as:
      - string key (from ``IntCodes``)
      - integer param value (from ``IntCodes``)
    ...
    >>> build(91, 7)
    SGR[91;7]
    >>> build(IntCodes.HI_CYAN, IntCodes.UNDERLINED)
    SGR[96;4]
    """
    result: List[int] = []
    ...
    for arg in args:
        if isinstance(arg, str):
            ...
            resolved_param = getattr(IntCodes, arg_mapped, None)  # @FIXME method
...
def color_indexed(color: int, bg: bool = False) -> SequenceSGR:
    ...
    return SequenceSGR(key_code, IntCodes._EXTENDED_MODE_256, color)
...
def color_rgb(r: int, g: int, b: int, bg: bool = False) -> SequenceSGR:
    ...
    key_code = IntCodes.BG_COLOR_EXTENDED if bg else IntCodes.COLOR_EXTENDED
    return SequenceSGR(key_code, IntCodes._EXTENDED_MODE_RGB, r, g, b)
...
class Seqs:
    ...
    NO_BOLD_DIM = SequenceSGR(IntCodes.NO_BOLD_DIM)       # there is no separate sequence for
...
sgr_parity_registry.register_single(IntCodes.BOLD, IntCodes.NO_BOLD_DIM)
sgr_parity_registry.register_single(IntCodes.DIM, IntCodes.NO_BOLD_DIM)
sgr_parity_registry.register_single(IntCodes.ITALIC, IntCodes.ITALIC_OFF)
sgr_parity_registry.register_single(IntCodes.UNDERLINED, IntCodes.UNDERLINED_OFF)
sgr_parity_registry.register_single(IntCodes.DOUBLE_UNDERLINED, IntCodes.UNDERLINED_OFF)
sgr_parity_registry.register_single(IntCodes.BLINK_SLOW, IntCodes.BLINK_OFF)
sgr_parity_registry.register_single(IntCodes.BLINK_FAST, IntCodes.BLINK_OFF)
sgr_parity_registry.register_single(IntCodes.INVERSED, IntCodes.INVERSED_OFF)
sgr_parity_registry.register_single(IntCodes.HIDDEN, IntCodes.HIDDEN_OFF)
sgr_parity_registry.register_single(IntCodes.CROSSLINED, IntCodes.CROSSLINED_OFF)
sgr_parity_registry.register_single(IntCodes.OVERLINED, IntCodes.OVERLINED_OFF)

for c in [*IntCodes.LIST_COLORS, *IntCodes.LIST_HI_COLORS]:
    sgr_parity_registry.register_single(c, IntCodes.COLOR_OFF)

for c in [*IntCodes.LIST_BG_COLORS, *IntCodes.LIST_BG_HI_COLORS]:
    sgr_parity_registry.register_single(c, IntCodes.BG_COLOR_OFF)


sgr_parity_registry.register_complex((IntCodes.COLOR_EXTENDED, 5),
                                     1, IntCodes.COLOR_OFF)
sgr_parity_registry.register_complex((IntCodes.COLOR_EXTENDED, 2),
                                     3, IntCodes.COLOR_OFF)
sgr_parity_registry.register_complex((IntCodes.BG_COLOR_EXTENDED, 5),
                                     1, IntCodes.BG_COLOR_OFF)
sgr_parity_registry.register_complex((IntCodes.BG_COLOR_EXTENDED, 2),
                                     3, IntCodes.BG_COLOR_OFF)
...
class Span:
    def __init__(self, *opening_params: str|int|SequenceSGR):
        ...
        Each argument can be specified as:
          ...
          - integer param value (defined in ``IntCodes``)
          - existing `SequenceSGR` instance (params will be extracted).
    ...
    def from_seq(cls, opening_seq: SequenceSGR = None, closing_seq: SequenceSGR = None,
        ...
        .. note ::
            `closing_seq` gets overwritten with :data:`.Seqs.RESET` if
...
class Spans:
    ...
    - ``NOOP.opening_seq`` and ``NOOP.closing_seq`` both returns `Seqs.NOOP`.
    
    >>> Spans.NOOP('text')
    'text'
    >>> Spans.NOOP.opening_str
    ''
    >>> Spans.NOOP.opening_seq
    SGR[^]
